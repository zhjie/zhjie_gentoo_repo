...
IUSE="+cxx debug ogg cpu_flags_x86_avx2 cpu_flags_x86_avx static-libs +include"
...
multilib_src_install_all() {
	doheader -r "${WORKDIR}"/"${P}"/include/share
	einstalldocs
	find "${ED}" -type f -name '*.la' -delete || die
	find "${ED}" -type f -name '*.am' -delete || die
	find "${ED}" -type f -name '*.in' -delete || die
}
